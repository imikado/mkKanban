<?php
class model_Sprints extends abstract_model{
	
	protected $sClassRow='row_Sprints';
	
	protected $sTable='kSprints';
	...
	public function findCountByProject($id){
		$oRow= $this->findOneSimple('SELECT count(id) as total FROM '.$this->sTable.' WHERE project_id=?',$id );
		return $oRow->total;
	}
	
	public function findAllByProject($project_id){
		return $this->findMany('SELECT * FROM '.$this->sTable.' WHERE project_id=?',$project_id );
	}
	
	
	public function addSprint($project_id_,$sTitle_){
		
		$iCount=$this->findCountByProject($project_id_);
		
		$oColumn=new row_Sprints;
		$oColumn->title=$sTitle_;
		$oColumn->project_id=$project_id_;
		$oColumn->position=$iCount+1;
		$oColumn->save();
	}
}

class row_Sprints extends abstract_row{
	
	protected $sClassModel='model_Sprints';
