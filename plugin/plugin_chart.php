...
class plugin_chart{
	...
	public function __construct($sType,$iWidth=null,$iHeight=null){
		...
		}else if($sType==self::LINEBYDATE){
			$this->oChart=new plugin_chartLineByDate($this->iWidth,$this->iHeight);
		}else{
			throw new Exception('sType non reconnu, attendu: (PIE,HISTO,LINES,BAR,LINESSCRUM)');
		}
	}
	...
	*/
	...
	public function setStartdate($startdate){
		$this->oChart->setStartdate($startdate);
		
	}
	public function setEnddate($enddate){
		$this->oChart->setEnddate($enddate);
...
class plugin_chartLineByDate extends abstract_pluginChart{
	...
	public function show(){
		...
		foreach($this->tData as $sGroup => $tDetail){
			foreach($tDetail['tPoint'] as $tPoint){
				...
				$tIndexData[$date][$sGroup]['label']=$tDetail['label'];
				$tIndexData[$date][$sGroup]['color']=$tDetail['color'];
				$tIndexData[$date][$sGroup]['y']=$y;
		...
		$nbJ=$oInterval->format('%d');
		
		plugin_debug::addSpy('$oStartdate',$oStartdate);
		plugin_debug::addSpy('$oEnddate',$oEnddate);

		plugin_debug::addSpy('$nbJ',$nbJ);
		
		$widthCase=($iWidth)/$nbJ;
		...
		foreach($this->tData as $sGroup => $tDetail){
			...
			$oDateCurrent=new Datetime($this->startdate);
			for($i=0;$i<=$nbJ;$i++){
				
				if(isset($tIndexData[ $oDateCurrent->format('Y-m-d') ][$sGroup] ) ){
					
					$tDetail=$tIndexData[ $oDateCurrent->format('Y-m-d') ][$sGroup];
					
					//if($sGroupD!=$sGroup) continue;
				
					$y=$tDetail['y'];
				
					$x2=($i*$widthCase)+$this->iMarginLeft;
					$y2=(1-($y-$iMinY)/($iMaxY-$iMinY))*$iHeight;
					...
					plugin_debug::addSpy('rect',$x3.' '.$y3);
				...
				$oDateCurrent->add(new DateInterval('P1D'));
		...
		foreach($this->tData as $sGroup => $tDetail){
			...
			foreach($tDetail['tPoint'] as $j => $tPoint){
				...
				$x2=($j*($iWidth/$iMaxCountX))+$this->iMarginLeft;
				$y2=(1-($y-$iMinY)/($iMaxY-$iMinY))*$iHeight;
		...
		*/
		...
		$oDateCurrent2=new Datetime($this->startdate);
		for($i=0;$i<=$nbJ;$i++){
			...
			$this->lineFromTo($x2,($this->iHeight-10),$x2,($this->iHeight-5) );
			$this->text($x2+2,($this->iHeight), $oDateCurrent2->format('d/m') );
			
			$oDateCurrent2->add(new DateInterval('P1D'));
		}
